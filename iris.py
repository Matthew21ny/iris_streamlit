...
#  予測モデルの構築
x = iris.data[:, [0, 2]]  # 全ての行の0番目と2番めの列を指定
y = iris.target
...
# インプットデータ（1行のデータフレーム）
value_df = pd.DataFrame({'data': 'data', 'sepal length (cm)': sepalValue,
                        'petal length (cm)': petalValue}, index=[0])
...
pred_df = pd.DataFrame(pred_probs, columns=[
                       'setosa', 'versicolor', 'virginica'], index=['probability'])
